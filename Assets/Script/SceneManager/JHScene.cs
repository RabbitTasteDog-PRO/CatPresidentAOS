...
public class JHScene : MonoBehaviour
{
    ...
    private List<JHLayer> _activeLayers = new List<JHLayer>();
    ///활성화 대기중인 레이어를 담아둔다. 순서대로 떠야 하므로 큐 처리..
    private List<JHLayer> _waitLayers = new List<JHLayer>();
    ...
    ///활성호 대기열에 레이어를 추가한다.
    public virtual void AddWaitLayer(JHLayer layer)
    {
    ...
    ///레이어를 추가한다.
    public virtual void AddLayer(JHLayer layer)
    {
    ...
    ///레이어를 삭제한다.
    public virtual void RemoveLayer(JHLayer layer)
    {
    ...
    public void RemoveLayerAll()
    {
        ...
        JHLayer[] deleteLayers = new JHLayer[_activeLayers.Count];
    ...
    public virtual void BackPressed()
    {
        ...
        {
            // Debug.Log("_activeLayers.Count : " + _activeLayers.Count);
            if (_activeLayers[_activeLayers.Count - 1].backKeyEnable)
            {
                // Debug.Log("_activeLayers[_activeLayers.Count-1].backKeyEnable : " + _activeLayers[_activeLayers.Count - 1].backKeyEnable);
    ...
    public bool IsActiveLayer(JHLayer layer)
    {
